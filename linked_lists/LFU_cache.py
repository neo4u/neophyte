...
# get()
# Through checking self._cache[key], we can get the node in O(1) time.
# Just performs self._update, then we can return the value of node.

# put()
# If `key` already exists in self._cache, we do the same operations as `get`, except
