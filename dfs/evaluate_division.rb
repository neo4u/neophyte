...
def build_graph(equations, values)
    equations.zip(values) do |edge, w|
        ...
        @weights[[a, b]], @weights[[b, a]] = w, 1.0 / w # We do 1.0 / w to get the float value, else it truncate the decimal value
    end
end

def dfs(src, dst, visited, path)
    # Return if:
    # 1. we've reached the destination, cuz then we're done
    # 2. if the src actually exists in the graph, cuz if not, then we've there is no path
    #    and hence we've to return -1.0 which is already the default values of @dfs_path
    ...
    @graph[src].each do |node|
        # we keep going deeper into the path as long as we hit the dst
        # and keep calulating the weight along the path
...
def dfs(src, dst, path, visited = Set.new())
    return -1.0 if !@graph.key?(src) || !@graph.key?(dst)
    # Return if:
    # 1. we've reached the destination, cuz then we're done
    # 2. if the src actually exists in the graph, cuz if not, then we've there is no path
    #    and hence we've to return -1.0 which is already the default values of @dfs_path
    return path if src == dst || visited.include?(src)
    visited.add(src)

    @graph[src].each do |node|
        # we keep going deeper into the path as long as we hit the dst
        # and keep calulating the weight along the path
        dfs(node, dst, path * @weights[[src, node]], visited)
    end
...
def find_path(src, dst)
    ...
    while !q.empty?
        ...
        @graph[node].each do |n|
            ...
            q.push([n, path * @weights[[node, n]]]) # We do 1.0 / w to get the float value, else it truncate the decimal value
        end
    end
