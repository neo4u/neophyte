...
assert_equal(move_zeroes([0,1,0,3,12]), [[0, 0, 1, 3, 12]])
assert_equal(move_zeroes([0,0,1]), [0,0,1])
...
# Keep the index of last index after non-zero element and each time you get a non-zero element swap with the index after last non-zero
# Time: O(n)
# Space: O(1)