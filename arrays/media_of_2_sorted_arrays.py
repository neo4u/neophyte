...
class Solution:
    ...
    def find_median_sorted_arrays_same_size(self, nums1, nums2):
        ...
        if m1 < m2:
            if n % 2 == 0:  self.find_median_sorted_arrays_same_size(nums1[mid - 1:], nums2[:mid + 1])
            else:           self.find_median_sorted_arrays_same_size(nums1[mid:], nums2[:mid + 1])
        else:
            if n % 2 == 0:  self.find_median_sorted_arrays_same_size(nums2[mid - 1:], nums1[:mid + 1])
            else:           self.find_median_sorted_arrays_same_size(nums2[mid:], nums1[:mid + 1])

    def median_sorted(self, a):
        n = len(a)
        mid = n // 2
        if n % 2 == 0:  (a[mid] + a[mid - 1]) / 2
        else:            a[n // 2]
...
nums1 = [1, 2]
nums2 = [3, 4]
assert sol.find_median_sorted_arrays_same_size(nums1, nums2) == 2.5

nums1 = [2, 3]
nums2 = [4, 5]
assert sol.find_median_sorted_arrays_same_size(nums1, nums2) == 3.5
