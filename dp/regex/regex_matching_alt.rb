...
def is_match(s, p)
    ...
    0.upto(n - 1) do |i|
        dp[0][i + 1] = dp[0][i - 1] if p[i] == '*' # We consider the case of 0 repitions of char at p[i - 1]
    end
    ...
    0.upto(m - 1) do |i|
        0.upto(n - 1) do |j|
            ...
            if p[j] == '*'
                dp[i + 1][j + 1] = dp[i + 1][j - 1] # 0 repititions case
...
#         dp[i][j] = dp[i-1][j]   // ex: in this case, a* counts as multiple a
