...
class Solution:
    def splitArray(self, nums: List[int], m: int) -> int:
        ...
        while l <= r:
            mid = (l + r) // 2
            if self.is_valid_max(nums, m, mid): # can you make at-most m sub-arrays with maximum sum at most mid
                ans, r = mid, mid - 1
            else:
                l = mid + 1
    ...
    # do we have to cut more than m times in order to respect the limit of mid being the max part
    def is_valid_max(self, nums, m, mid):
...
class Solution:
    def splitArray(self, nums: List[int], m: int) -> int:
        ...
        while l < r:
            mid = (l + r) // 2
            if self.is_valid(mid): r = mid
            else: l = mid + 1

        return l

    def is_valid(self, mid):
        curr_sum, cuts = 0, 0
